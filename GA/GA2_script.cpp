...
// random number generator for circuit size
...
// random number generator for genetic operation
...
vector<Circuit> crossOver(Circuit c1, Circuit c2){
	...
	// Re-count the gates in the new circuit
...
// Choose a random gate, change this gate to another random gate and choose
// random inputs to this gate. Then go through and make sure later gates arent
// effected and update them.
Circuit mutateCircuit(Circuit c) {
	...
	// If we have two NOT gates, we cannot choose to switch to a NOT gate
	...
	// Go through and "refresh" the values of all gates after the randomly
	// selected gate
...
// Choose whether we mutate or crossover
...
void checkCircuit(Circuit &currentCirc){
	...
	// Check all gate solutions and match gates to solutions
